...
public class AuthServiceTest {
   ...
   * - Configures the `userRepo` mock to return the mock `User` object when `getUserByUsername` is called.
   ...
   * - Configures the `authManager` mock to return a mock `Authentication` object when `authenticate` is called.
   * - Configures the `authentication` mock to return `true` when `isAuthenticated` is called.
   */
    @BeforeEach
    public void setUp() {
        User user = new User();
